...
export default function SocialFloating({ menuOpen = false }: { menuOpen?: boolean }) {
  ...
  return (
    <div className={`fixed bottom-6 ${isOpen ? 'right-5' : 'right-0'} z-50 transition-all duration-300`}>
      {isOpen ? (
        <div className={`flex flex-col items-center space-y-3 origin-bottom transition-all duration-500 ${isAnimating ? 'scale-y-0 opacity-0' : 'scale-y-100 opacity-100'}`}>
        ...
        >
          <div className="flex flex-col items-center space-y-0.5 lg:space-y-1.5">
            {'contact'.split('').map((c, i) => (<span key={`c-${i}`}>{c}</span>))}
            <div className="h-2" />
            {'us'.split('').map((c, i) => (<span key={`u-${i}`}>{c}</span>))}
