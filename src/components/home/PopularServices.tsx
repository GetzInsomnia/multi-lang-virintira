...
// Animation Variants (Scroll-Triggered)
const itemAnim = {
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 mx-auto w-full max-w-6xl">
        ...
        <div
          className="mt-12 grid auto-rows-[320px] gap-5 sm:auto-rows-[280px] sm:grid-cols-2 lg:auto-rows-[240px] lg:grid-cols-4"
        >
          {services.map((item, index) => {
            ...
            return (
              <motion.div
                ...
                className={`relative h-full ${isHeroCard ? 'sm:col-span-2 lg:col-span-2 lg:row-span-2' : ''}`}
              >
                <Link
                  href={`/services/${item.categorySlug}/${item.serviceSlug}`}
                  className="group relative isolate flex h-full flex-col overflow-hidden rounded-2xl border border-[#f3dede] bg-white shadow-[0_8px_30px_rgb(167,9,9,0.15)] transition-shadow duration-300 ease-out hover:shadow-[0_8px_30px_rgb(167,9,9,0.30)]"
                >
                  <div
                    className={`relative w-full overflow-hidden ${isHeroCard ? 'aspect-[4/3]' : 'aspect-[16/9]'
                      }`}
                  >
                    <Image
                      src={item.imagePath}
